  ...
  try {
    ...
    try {
      ...
      const pfp = await conn
        ...
        .catch((_) => "https://i.imgur.com/tt3q2RC.jpg");
      ...
      if (config) {
        if (!"premium" in config) config.premium = [];
        if (!"owner" in config) config.owner = [];
      } else {
        db.data.config = {
          premium: [""],
          owner: [""],
    ...
    const isROwner = [
      ...
      // db.data.config.owner.map(([number]) => number),
    ]
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        try {
          if (!isOwner) {
            if (
              m.isGroup &&
              new Date() - global.db.data.chats[m.chat].delay < 1
            )
              return;
            if (
              !m.isGroup &&
              new Date() - global.db.data.users[m.sender].delay < 1
            )
              return;
          }
    ...
    try {
      if (!opts["noprint"])
        await (
          await import(`./lib/print.js?update=${Date.now()}`)
        ).default(m, this);
    } catch (e) {
...
global.dfail = (type, m, conn) => {
  ...
  if (msg)
    return conn.reply(m.chat, msg, {
      ...
      message: { pollCreationMessage: { name: `Simple Bot WhatsApp` } },
    });
};
