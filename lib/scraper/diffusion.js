...
const diffusion2 = (
  ...
  new Promise((resolve, reject) => {
    const options = {};
    axios("https://api.prodia.com/generate", {
      method: "GET",
      params: {
        new: "true",
        prompt,
      ...
      .then(async (response) => {
        ...
        while (p == true) {
          const { data } = await axios.get(
            `https://api.prodia.com/job/${response.data.job}`
          );
        ...
        const result = {
          img: `https://images.prodia.xyz/${response.data.job}.png`,
        };
        resolve(result);
      })
      .catch((err) => reject(err));
  });
// diffusion2("milf,anime,armpit");
export { diffusion, diffusion2 };
// diffusion("anime")
//   .then((result) => {
//     console.log(result);
//   })
