...
import { ehe } from "./leaderboard/app.js";
...
global.db = new Low(
  opts["db"]
    ? new CloudDBAdapter(
        "https://closedhatefuldecagon.andikavikarmuha.repl.co/"
      )
...
// bates
global.authFile = global.opts["single"]
  ? `${global.opts._[0] || "session"}.data.json`
  : "session";
const { state, saveState, saveCreds } = global.opts["single"]
  ? await useSingleFileAuthState(global.authFile)
...
const connectionOptions = {
  ...
  getMessage: async (key) =>
    (conn.loadMessage(key.id) || store.loadMessage(key.id) || {}).message || {
      conversation: null,
    }, // 'Please send messages again' }
};
// bates
...
global.reloadHandler = async function (restatConn) {
  ...
  conn.credsUpdate = global.opts["single"]
    ? saveState.bind(global.conn)
...
http
  .createServer((req, res) => {
    ...
    } else if (method === "GET" && url === "/apacoba") {
